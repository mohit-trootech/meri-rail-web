...
const UserProfile = ({ details, updateUserDetails }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    toggle && updateUserDetails(new FormData(e.target));
  };
  return (
    <section class="p-4">
      <div class="flex flex-row justify-between items-center w-full gap-10">
        <h1 className="font-bold text-xl uppercase">User Details</h1>
        <div
          className="tooltip tooltip-left"
          data-tip={
            toggle ? "Click to View Details" : "Click to Update Details"
          }
        >
          <button
            className={
              (toggle && "btn btn-sm btn-warning") || "btn btn-sm btn-info"
            }
      ...
      <form method="POST" onSubmit={handleSubmit}>
        <div class="md:flex mb-8">
          <div class="md:w-1/3">
            ...
            <p class="text-xs font-light text-red">User Personal Details</p>
          </div>
